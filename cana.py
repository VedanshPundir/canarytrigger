...
def generate_url_token():
    ...
    # Embed the URL into the image
    from PIL import Image
    import stepic

    image = Image.open("static/base.jpeg")  # Make sure this exists
    ...
    return render_template("generate_token.html", url=url, image_url=f"/download-stego/{token}")
...
def download_stego(token):
    ...
    # Suppress alert if sender is downloading it
    with open("sent_tokens.txt", "r") as f:
        sent_tokens = f.read().splitlines()
    if token in sent_tokens:
        print(f"⚠ Suppressed alert for sender download of token: {token}")
